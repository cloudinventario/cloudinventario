...
class CloudInventarioAzureMysql(CloudInvetarioResource):
    ...
    def _login(self, credentials):
        try:
            ...
            logging.info("logging config for AzureMysql={}".format(
    ...
    def _fetch(self):
        data = []
        for sql in list(self.mysql_client.servers.list()):
            data.append(self._process_resource(sql.as_dict()))

        logging.info("Collected {} mysql".format(len(data)))
        return data

    def _process_resource(self, sql):
        logging.info("new AzureMysql name={}".format(sql.get('name')))
        data = {
            "id": sql.get('id'),
            "name": sql.get('name'),
            "type": sql.get('type'),
